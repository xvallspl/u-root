...
func alignUp(val uint32) uint32 {
	mask := uint32(os.Getpagesize() - 1)
	return (val + mask) &^ mask
}

// bufAlignUp pads buf to a page boundary.
func bufAlignUp(buf *bytes.Buffer) error {
			...
			ranges: []mutibootModuleRange{
				{
					...
					numPages:     alignUp(mm.End-mm.Start) / uint32(os.Getpagesize()),
				},
			},
		})
	}
